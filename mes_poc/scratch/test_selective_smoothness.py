...
class ConstrainedBodyModel:
    ...
    def fit_shape(self, front_silhouette, side_silhouette, height_cm, target_neck_w=None, epochs=250):
        ...
        def closure():
            ...
            # SELECTIVE SMOOTHNESS
            diff2_w = (w_cm[2:] - 2 * w_cm[1:-1] + w_cm[:-2]) ** 2
            diff2_d = (d_cm[2:] - 2 * d_cm[1:-1] + d_cm[:-2]) ** 2
            
            smooth_weights = torch.ones(98, dtype=torch.float32)
            # Slices 75 to 84 (representing i+1, so slices 76 to 85)
            smooth_weights[74:85] = 0.01 # Reduce transition penalty
            
            smooth_w = torch.mean(diff2_w * smooth_weights)
            smooth_d = torch.mean(diff2_d * smooth_weights)
...
def evaluate_accuracy(rbf_div, neck_sp):
    print(f"\nEvaluating Config with Selective Smoothness: RBF Divisor={rbf_div}, Neck Spread={neck_sp}")
    all_diffs = []
